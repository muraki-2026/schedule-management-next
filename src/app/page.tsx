...
function timeValue(v?: string | null) { return v ? v.slice(0, 5) : ''; }
...
export default function Home() {
  ...
  async function saveSchedule(e: React.FormEvent) {
    ...
    const previousSchedule = draft.id ? schedules.find((x) => x.id === draft.id) : null;
    const payload = { title: draft.title, schedule_date: draft.schedule_date, start_time: timeValue(draft.start_time) || null, end_time: timeValue(draft.end_time) || null, owner_id: null, assignee_id: assigneeId, assignee_text: draft.assignee_id === '__manual__' ? (draft.assignee_manual || null) : null, schedule_type_id: scheduleTypeId, schedule_type_text: draft.schedule_type_id === '__manual__' ? (draft.schedule_type_manual || null) : null, place: placeId, place_text: draft.place === '__manual__' ? (draft.place_manual || null) : null, memo: draft.memo || null, status: draft.status, created_by: currentUser?.id || null };
    ...
    if (res.error) setMessage(res.error.message); else { const scheduleId = (res.data as { id?: string } | null)?.id ?? draft.id ?? null; const shouldSendEmail = !draft.id ? draft.email_send : draft.email_send && draft.email_send_on_update; if (shouldSendEmail) await sendScheduleEmail(scheduleId, draft, payload, Boolean(draft.id), previousSchedule ?? undefined); setDraft(null); await loadData(); }
  }
  function scheduleEmailText(schedule: Partial<Schedule> & { title: string; schedule_date: string }, isUpdate = false, previous?: Schedule) {
    const valueSet = (src: Partial<Schedule> & { title?: string; schedule_date?: string }) => {
      const assignee = src.assignee_id ? items.find((i) => i.id === src.assignee_id)?.name : src.assignee_text;
      const type = src.schedule_type_id ? items.find((i) => i.id === src.schedule_type_id)?.name : src.schedule_type_text;
      const place = src.place ? items.find((i) => i.id === src.place)?.name : src.place_text;
      return {
        title: src.title || '',
        date: src.schedule_date || '',
        time: (src.start_time?.slice(0,5) || '終日') + (src.end_time ? '〜' + src.end_time.slice(0,5) : ''),
    ...
    const current = valueSet(schedule);
    const before = previous ? valueSet(previous) : null;
    const mark = (key: keyof typeof current) => isUpdate && before && before[key] !== current[key] ? '※' : '';
    return [
      isUpdate ? '以下の予定が更新されました。' : '以下の予定が登録されました。',
      '',
      mark('title') + '件名：' + current.title,
      mark('date') + '日付：' + current.date,
      mark('time') + '時間：' + current.time,
      mark('assignee') + '担当者：' + current.assignee,
...
function ScheduleBadge({ s, assignees, types, locations }: { s: Schedule; assignees: MasterItem[]; types: MasterItem[]; locations: MasterItem[] }) {
  const type = scheduleTypeName(s, types);
  const assignee = assigneeName(s, assignees);
  const place = placeName(s, locations);
  const memo = s.memo || '';
  return <span className={`event undefined`} style={{ background: scheduleColor(s, assignees, types) }}><span className="event-line event-main"><span className="event-time">{s.start_time?.slice(0,5) || '終日'}</span>{assignee && <span className="event-assignee">【{assignee}】</span>}{place && <span className="event-muted">{place}</span>}</span><span className="event-line event-sub">{type && <span className="event-type">（{type}）</span>}<span className="event-title">{s.title}</span>{memo && <span>{memo}</span>}</span></span>;
}
...
function Calendar({ mode, setMode, monthLayout, setMonthLayout, viewDate, setViewDate, schedules, users, assignees, types, locations, open, query, setQuery, exportCsv, calendarWidth, setCalendarWidth }: { mode: CalendarMode; setMode: (m: CalendarMode) => void; monthLayout: MonthLayout; setMonthLayout: (m: MonthLayout) => void; viewDate: Date; setViewDate: (d: Date) => void; schedules: Schedule[]; users: UserProfile[]; assignees: MasterItem[]; types: MasterItem[]; locations: MasterItem[]; open: (s: DraftSchedule) => void; query: string; setQuery: (v: string) => void; exportCsv: () => void; calendarWidth: number; setCalendarWidth: (v: number) => void }) {
  ...
  const renderDayCell = (d: Date, compact = false) => {
    ...
    return <div key={date} onDoubleClick={() => open({ title: '', schedule_date: date, status: 'planned' })} className={className}><b>{label}</b>{ev.map((x) => <div key={x.id} onClick={() => open({ ...x })}><ScheduleBadge s={x} assignees={assignees} types={types} locations={locations} /></div>)}</div>;
  };
  const renderTimedGrid = () => {
    ...
    return <div className="border-t border-gray-200"><div className="grid" style={{ gridTemplateColumns: columns }}><div className="border-b border-r border-gray-200 bg-gray-50 p-2 text-center text-sm font-bold">時刻</div>{targetDays.map((d) => { const dow = d.getDay(); const cls = 'p-2 text-center font-bold border-b border-r border-gray-200 ' + (dow === 0 ? 'text-red-500 bg-rose-50' : dow === 6 ? 'text-blue-500 bg-blue-50' : 'text-gray-600 bg-gray-50'); return <div key={ymd(d)} className={cls}>{ymd(d)} ({['日','月','火','水','木','金','土'][dow]})</div>; })}<div className="border-b border-r border-gray-200 bg-amber-50 p-2 text-center text-sm font-bold">終日</div>{targetDays.map((d) => { const date = ymd(d); const allDay = schedules.filter((x) => x.schedule_date === date && !x.start_time && !x.end_time); return <div key={'all-' + date} className="min-h-16 border-b border-r border-gray-200 bg-amber-50 p-2">{allDay.length === 0 ? <span className="text-xs text-gray-400">-</span> : allDay.map((x) => <div key={x.id} onClick={() => open({ ...x })}><ScheduleBadge s={x} assignees={assignees} types={types} locations={locations} /></div>)}</div>; })}{hourSlots.map((h) => <Fragment key={h}><div className="min-h-20 border-b border-r border-gray-200 bg-gray-50 p-2 text-center text-sm font-bold text-gray-600">{pad(h)}:00</div>{targetDays.map((d) => { const date = ymd(d); const ev = schedules.filter((x) => x.schedule_date === date && x.start_time && Number(x.start_time.slice(0,2)) === h).sort((a,b)=>(a.start_time||'').localeCompare(b.start_time||'')); return <div key={date + '-' + h} onDoubleClick={() => open({ title: '', schedule_date: date, start_time: pad(h) + ':00', end_time: pad((h + 1) % 24) + ':00', status: 'planned' })} className="min-h-20 border-b border-r border-gray-200 p-2">{ev.map((x) => <div key={x.id} onClick={() => open({ ...x })}><ScheduleBadge s={x} assignees={assignees} types={types} locations={locations} /></div>)}</div>; })}</Fragment>)}</div></div>;
  };
