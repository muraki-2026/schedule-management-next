...
function ScheduleBadge({ s, assignees, types, locations }: { s: Schedule; assignees: MasterItem[]; types: MasterItem[]; locations: MasterItem[] }) {
  ...
  const memo = s.memo || '';
  return <span className={`event undefined`} style={{ background: scheduleColor(s, assignees, types) }}><span className="event-line event-main"><span className="event-time">{s.start_time?.slice(0,5) || '終日'}</span>{assignee && <span className="event-assignee">【{assignee}】</span>}</span><span className="event-line event-sub">{type && <span>（{type}）</span>}<span className="event-title">{s.title}</span>{place && <span>{place}</span>}{memo && <span>{memo}</span>}</span></span>;
}
