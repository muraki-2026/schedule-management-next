...
export default function Home() {
  ...
  async function ensureMasterItem(def: MasterDefinition | undefined, name: string | undefined, fallbackColor = '#f3f4f6') {
    const itemName = name?.trim();
    if (!itemName) return null;
    const existing = items.find((i) => i.master_definition_id === def?.id && i.name === itemName);
    if (existing) return existing.id;
    if (!def?.id) { setMessage('対象マスタが見つからないため、手入力項目を追加できません。'); return null; }
    if (!supabase) {
      const id = crypto.randomUUID();
      setItems((v) => [...v, { id, master_definition_id: def.id, name: itemName, value: fallbackColor, sort_order: v.length + 1, active: true }]);
      return id;
    }
    const { data, error } = await supabase.from('master_items').insert({ master_definition_id: def.id, name: itemName, value: fallbackColor, sort_order: items.filter((i) => i.master_definition_id === def.id).length + 1, active: true }).select('id').single();
  ...
  async function saveSchedule(e: React.FormEvent) {
    ...
    const scheduleTypeId = draft.schedule_type_id === '__manual__' ? await ensureMasterItem(scheduleTypeDef, draft.schedule_type_manual, colorSamples[0]) : draft.schedule_type_id || null;
    const assigneeId = draft.assignee_id === '__manual__' ? await ensureMasterItem(assigneeDef, draft.assignee_manual, colorSamples[5]) : draft.assignee_id || null;
    const placeId = draft.place === '__manual__' ? await ensureMasterItem(locationTypeDef, draft.place_manual, colorSamples[2]) : draft.place || null;
...
function AssigneeSchedulePanel({ schedules, assignees, types, open }: { schedules: Schedule[]; assignees: MasterItem[]; types: MasterItem[]; open: (s: DraftSchedule) => void }) {
  ...
  return <aside className="card p-4 lg:sticky lg:top-4 lg:max-h-[calc(100vh-2rem)] lg:overflow-auto"><h3 className="mb-3 text-lg font-bold">担当者別予定</h3>{assignees.length === 0 ? <p className="rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm text-gray-500">表示対象の担当者がありません。</p> : <div className="grid gap-4">{assignees.map((a) => { const mine = futureSchedules.filter((s) => s.assignee_id === a.id).slice(0, 5); return <div key={a.id} className="rounded-xl border border-gray-200 bg-white p-3"><h4 className="font-bold">{a.name}</h4><div className="mt-2 grid gap-2">{mine.length === 0 ? <p className="text-sm text-gray-500">予定はありません。</p> : mine.map((s) => { const type = types.find((t) => t.id === s.schedule_type_id); return <button key={s.id} onClick={() => open({ ...s })} className="text-left rounded-lg border border-gray-100 bg-gray-50 p-2 hover:bg-blue-50"><div className="text-sm font-bold">{s.title}</div><div className="text-xs text-gray-600">{s.schedule_date} {s.start_time?.slice(0,5) || '終日'}{s.end_time ? `〜${s.end_time.slice(0,5)}` : ''}</div>{type && <div className="mt-1 inline-block rounded px-2 py-0.5 text-xs" style={{ background: type.value || '#f3f4f6' }}>{type.name}</div>}</button>; })}</div></div>; })}</div>}<p className="mt-3 text-xs text-gray-500">各担当者ごとに今日以降の予定を最大5件表示します。表示対象は「担当者別予定表示」マスタで絞り込みできます。</p></aside>;
}
function Calendar({ mode, setMode, viewDate, setViewDate, schedules, users, assignees, types, open, query, setQuery, exportCsv }: { mode: CalendarMode; setMode: (m: CalendarMode) => void; viewDate: Date; setViewDate: (d: Date) => void; schedules: Schedule[]; users: UserProfile[]; assignees: MasterItem[]; types: MasterItem[]; open: (s: DraftSchedule) => void; query: string; setQuery: (v: string) => void; exportCsv: () => void }) {
