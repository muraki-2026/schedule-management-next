...
type DraftSchedule = Partial<Schedule> & { title: string; schedule_date: string; status: Schedule['status']; schedule_type_manual?: string; assignee_manual?: string; place_manual?: string; email_send?: boolean; email_target_type?: 'recipient' | 'group'; email_recipient_id?: string; email_group_id?: string };
...
export default function Home() {
  ...
  async function saveSchedule(e: React.FormEvent) {
    ...
    if (res.error) setMessage(res.error.message); else { const scheduleId = (res.data as { id?: string } | null)?.id ?? draft.id ?? null; if (draft.email_send) await sendScheduleEmail(scheduleId, draft, payload); setDraft(null); await loadData(); }
  }
  function scheduleEmailText(schedule: Partial<Schedule> & { title: string; schedule_date: string }) {
    ...
    return ['以下の予定が登録・更新されました。', '', '件名：' + schedule.title, '日付：' + schedule.schedule_date, '時間：' + ((schedule.start_time?.slice(0,5) || '終日') + (schedule.end_time ? '〜' + schedule.end_time.slice(0,5) : '')), '担当者：' + (assignee || '未設定'), '予定区分：' + (type || '未設定'), '場所：' + (place || '未設定'), 'メモ：' + (schedule.memo || '')].join('\n');
  }
  ...
  async function sendScheduleEmail(scheduleId: string | null, draft: DraftSchedule, saved: Partial<Schedule> & { title: string; schedule_date: string }) {
  ...
  async function addEmailRecipient() {
    ...
    const name = prompt('送信先名を入力してください');
    if (!name) return;
    const email = prompt('メールアドレスを入力してください');
    if (!email) return;
    const { error } = await supabase.from('email_recipients').insert({ name, email, active: true, sort_order: emailRecipients.length + 1 });
...
function Masters({ isAdmin, departments, users, masters, items, emailRecipients, emailGroups, emailGroupMembers, addEmailRecipient, addEmailGroup, addEmailGroupMember, removeEmailGroupMember, deleteEmailRecipient, deleteEmailGroup, edit }: { isAdmin: boolean; departments: Department[]; users: UserProfile[]; masters: MasterDefinition[]; items: MasterItem[]; emailRecipients: EmailRecipient[]; emailGroups: EmailGroup[]; emailGroupMembers: EmailGroupMember[]; addEmailRecipient: () => void; addEmailGroup: () => void; addEmailGroupMember: (groupId: string) => void; removeEmailGroupMember: (groupId: string, recipientId: string) => void; deleteEmailRecipient: (id: string) => void; deleteEmailGroup: (id: string) => void; edit: (e: MasterEdit) => void }) {
